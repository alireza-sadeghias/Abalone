...
public class SortResults {
    private final static Logger logger = LogManager.getLogger(SortResults.class);
    ...
    public static ArrayList<double[][]> sorting(){
        ...
        for(int j=1; j<=3; j++){
            for(int i=1; i<=EvolutionaryAlgo.initialGenerationSize; i++){
                compare = ReadResult.readIn(System.getProperty("user.dir") + ReadMatrix.slash + "\\src\\main\\java\\ir\\alireza\\sadeghi\\shiraz\\ai" +ReadMatrix.slash +"Results"+ReadMatrix.slash + "AIResult" + ReadMatrix.gen +"_" +i+"_" +j + ".txt");
